 ...
 * 解析器接�?
 * */
public abstract class YanxiuBaseParser<T extends YanxiuBaseBean, D> {
	...
	public YanxiuBaseParser(int from){
		this.from = from ;
	}
...
//			}
	 ...
	 * 如果独立接口，请实现为返�?  true，否则不会进入解析方法，并抛出JsonCanNotParseException
	 * */
	protected abstract boolean canParse(String data);
	
	/**
	 * 针对不同的接口类型，给parse方法吐出不同的数�?
	 * */
