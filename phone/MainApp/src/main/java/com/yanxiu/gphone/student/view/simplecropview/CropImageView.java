package com.yanxiu.gphone.student.view.simplecropview;
...
public class CropImageView extends ImageView {
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle,
                                 float mDensity) {
        TypedArray ta = context.obtainStyledAttributes(attrs, com.isseiaoki.simplecropview.R.styleable.scv_CropImageView,
                defStyle, 0);
        Drawable drawable;
        ...
        try {
            ...
            mBackgroundColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_background_color,
                    TRANSPARENT);
            mOverlayColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_overlay_color,
                    TRANSLUCENT_BLACK);
            ...
            mGuideColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_color,
                    TRANSLUCENT_WHITE);
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_show_mode,
                        1) == mode.getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_show_mode,
                        1) == mode.getId()) {
            ...
            mTouchPadding = ta.getDimensionPixelSize(
                    com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_touch_padding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(
                    com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_min_frame_size,
                    (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight = ta.getDimensionPixelSize(
                    com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_frame_stroke_weight,
                    (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight = ta.getDimensionPixelSize(
                    com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_stroke_weight,
            ...
            mIsAnimationEnabled = ta.getBoolean(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_animation_enabled,
                    true);
            mAnimationDurationMillis = ta.getInt(
                    com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_animation_duration,
                    DEFAULT_ANIMATION_DURATION_MILLIS);
            mIsHandleShadowEnabled = ta.getBoolean(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_shadow_enabled, true);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ta.recycle();
    ...
    public void startLoad(Uri sourceUri, LoadCallback callback) {
        ...
        mExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                //mExifRotation = Utils.getExifRotation(Utils.getFileFromUri(getContext(), mSourceUri));
