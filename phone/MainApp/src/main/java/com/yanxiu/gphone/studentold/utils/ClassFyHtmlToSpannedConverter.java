...
import com.yanxiu.gphone.studentold.HtmlParser.MyImageSpan;
import com.yanxiu.gphone.studentold.HtmlParser.PictureImageSpan;
import com.yanxiu.gphone.studentold.HtmlParser.Utils.Utils;
 ...
 */
public class ClassFyHtmlToSpannedConverter implements ContentHandler {
    ...
    public ClassFyHtmlToSpannedConverter(Context context, String source, ImageGetterListener imageGetterListener, MyHtml.TagHandler tagHandler, HtmlParser parser, Object object, ImageSpanOnclickListener listener) {
...
//            handleP(mSpannableStringBuilder);
    ...
    private static void startClozz(SpannableStringBuilder text){
        int len = text.length();
        text.setSpan("", len, len, Spannable.SPAN_MARK_MARK);
    }

    private void endClozz(SpannableStringBuilder text){
        int len = text.length();
    ...
    private void startImg(SpannableStringBuilder text, Attributes attributes, ImageGetterListener img) {
        ...
        if (d == null) {
            d = ContextCompat.getDrawable(context,R.drawable.advice_icon);
            d.setBounds(0, 0, d.getIntrinsicWidth(), d.getIntrinsicHeight());
        }
        ...
        if (where != len) {
            ...
            if (!TextUtils.isEmpty(f.mColor)) {
                if (f.mColor.startsWith("@")) {
                    ...
                    int c = Utils.getHtmlColor(f.mColor);
                    if (c != -1) {
