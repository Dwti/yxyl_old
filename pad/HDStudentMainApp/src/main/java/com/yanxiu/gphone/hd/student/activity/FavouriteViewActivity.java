...
import com.yanxiu.gphone.hd.student.requestTask.RequestDelFavouriteListTask;
...
//    private ArrayList<String> delQuestionTmpList = new ArrayList<String>();
    ...
    public void initData() {
        ...
        if (dataSources != null && dataSources.getData() != null) {
            ...
            tvPagerCount.setText(" / " + String.format(this.getResources().getString(R.string.pager_count), String.valueOf((adapter.getTotalCount()>pageCount ? adapter.getTotalCount
                    () : pageCount))));
...
//                LogInfo.log("haitian", "currentTotal =" + (currentTotal - YanXiuConstant.YX_PAGESIZE_CONSTANT + 1) + "----pageCount=" + pageCount);
    ...
    protected void onDestroy() {
        super.onDestroy();
        if(delQuestionTmpMap.size() >0){
            EventBus.getDefault().post(new UpdateCollectBean());
        }
    }
}