...
import com.yanxiu.gphone.hd.student.adapter.SubjectVersionSortAdapter;
...
//        mListView.setDivider(new BitmapDrawable(mBitmap));
        initAnim();
        // 生成PopupWindow对象
        popupWindow = new SortPopupWindow(popupView, RightContainerUtils.getInstance().getContainerWidth(),
                LayoutParams.MATCH_PARENT, true, onSortDismissListener);


        LogInfo.log("geny", "RightContainerUtils.getInstance().getContainerWidth()" + RightContainerUtils.getInstance().getContainerWidth());
...
//        mListView.setAnimation(animTopIn);
        ...
        popupWindow.showAsDropDown(parentView, 0, CommonCoreUtil.dipToPx(6));
    }
