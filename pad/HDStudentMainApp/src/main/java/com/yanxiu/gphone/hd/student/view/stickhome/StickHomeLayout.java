...
//        });
        ...
        flLeftMiddleContainter = (FrameLayout) this.findViewById(R.id.fl_left_middle_containt);
        flRightContainter = (FrameLayout) this.findViewById(R.id.fl_right_containt);
        flRightMiddleContainter = (FrameLayout) this.findViewById(R.id.fl_right_middle_containt);
    ...
    private void showContainterView(POSITION_TYPE type, SubjectVersionBean.DataEntity subjectData){
        ...
        switch (type) {
            ...
            case RIGHT_MIDDLE_TYPE:
                flRightMiddleContainter.addView(containter);
                break;
            case LEFT_MIDDLE_TYPE:
                flLeftMiddleContainter.addView(containter);
                break;
        }
                ...
                if (mAdapter.getItem(position).getData() != null) {
                    ...
                    if (!TextUtils.isEmpty(editionName) && editionName.equals("未选择")) {
                        ...
                        if(null != onItemClickListener){
                            onItemClickListener.selectionEntity(mAdapter.getItem(position).getName(),mAdapter.getItem(position));
                        }
...
//                                mAdapter.getItem(position));
    ...
    private void selectItem(int position, View view){
        ...
        }else if (position % ROW_NUM == 1) {
            type = POSITION_TYPE.LEFT_MIDDLE_TYPE;
        }else if (position % ROW_NUM == 2) {
            type = POSITION_TYPE.RIGHT_MIDDLE_TYPE;
        }else if (position % ROW_NUM ==3) {
    ...
    private void dismissContaintner(){
        ...
        flRightMiddleContainter.removeAllViews();
        flLeftMiddleContainter.removeAllViews();
...
//    private String editionId;
    ...
    public interface OnItemClickListener{
        ...
        void selectionEntity(String name,SubjectVersionBean.DataEntity entity);
    }
    ...
    public void recycleView(){
        ...
        viewMark=null;
        if(containter!=null){
            containter.recycleView();
            containter.removeAllViews();
            containter=null;
        }
        currentDataEntity=null;
        dataEntity=null;
        if(flLeftContainter!=null){
            flLeftContainter.removeAllViews();
            flLeftContainter=null;
        }
        if(flRightContainter!=null){
            flRightContainter.removeAllViews();
            flRightContainter=null;
        }
        if(flLeftMiddleContainter!=null){
            flLeftMiddleContainter.removeAllViews();
            flLeftMiddleContainter=null;
        }

        if(flRightMiddleContainter!=null){
            flRightMiddleContainter.removeAllViews();
            flRightMiddleContainter=null;
        }
